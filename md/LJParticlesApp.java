...
public class LJParticlesApp extends AbstractSimulation {
  ...
  public void reset() {
    ...
    control.setAdjustableValue("dt", 0.002); // Smaller dt might be needed for stiff walls

 // --- NEW Wall Controls & Logic for Central Forces ---
 control.setValue("useRepulsiveWalls", true); // Default to using wall confinement
 control.setValue("wallStiffness", 200.0);   // Default wall stiffness

 boolean wallsEnabled = control.getBoolean("useRepulsiveWalls"); // Get current or default
 if (wallsEnabled) {
     // If walls are on, central forces might be undesired or act differently
     control.setValue("springConstant", 0.0);  // Turn off central spring by default if walls are on
     control.setValue("gravityForce", 0.0);    // Turn off central gravity if walls are on
 } else {
     // If walls are off, use user's original defaults for central forces
     control.setValue("springConstant", 30.0);
     control.setValue("gravityForce", 0.89);
 }
 control.setValue("dampingCoeff", 50.0); // Reduced damping from 200 for more visible dynamics with walls
                                         // User's original 200 is very high and might make particles stick to walls.
 // --- END NEW ---
